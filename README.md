...
- `Unified asset discovery`: one discovery entry that shows datasets, code, benchmarks, research map, reading path, and representative papers together
...
`GITHUB_TOKEN` is strongly recommended for topic and area exploration, because GitHub's authenticated API limits are much more practical than anonymous search limits.
...
`POST /api/explore_assets` now powers one unified discovery entry in the UI. The response combines:

- asset-focused signals such as datasets, methods, benchmark assets, and repositories
- area-focused signals such as overview, representative papers, reading path, and subdirection layers

The backend still uses internal query interpretation so narrow tasks and broader areas are ranked appropriately, but the UI now shows both views together.
